
<button id = 'toggle-modal-button' type="button" class="hidden" data-toggle="modal" data-target="#myModal" data-backdrop="false">Open Modal</button>

<!-- Modal -->
<div id="myModal" class=" search-modal modal fade" role="dialog" >
    <!-- Modal content-->
    <div id = "search-modal-content" class=" fade col-md-offset-1 col-md-5 modal-content pushed-down-content">
      <div class="modal-header">
        <h4 id = "search-title" class="modal-title">Shkruaj që të kërkosh!</h4>
      </div>
      <div id = "modal-body" class="modal-body">

        <!-- Ajax returns questions and resources list here  -->

      </div>
    </div>

</div>
